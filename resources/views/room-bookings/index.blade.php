<x-layout>
  ...
  <div class="flex flex-wrap">
    @if ($roomBookings->isEmpty())
      <p class="text-zinc-600">no bookings...</p>
    @endif  
    ...
    <div class="block h-52 w-1/2 border border-gray-300 rounded-lg flex flex-col p-2 gap-2 bg-white/30">
      ...
      <div class="p-3 flex justify-between border-b-2 border-gray-200">
        ...
        <div class="flex flex-col items-center">
          <p class="font-bold">Floor No. <span class="text-base">{{ substr($roomBooking->room['code'], 0, 2) }}</span></p>
          <p class="font-bold">Number No. <span class="text-base">{{ substr($roomBooking->room['code'], -2) }}</span></p>
        </div>
      </div>
